...
function SkillDetail() {
  ...
  return (
    <main className="mx-auto max-w-[1100px] px-6 pb-10 w-full">
      ...
      <div className="panel p-6 mt-4"><div className="flex flex-wrap items-end gap-4"><div className="flex-1 min-w-[260px]"><label className="kicker !text-[9px]" htmlFor="agent-key">{t("skill.probeLabel")}</label><input id="agent-key" type="password" autoComplete="off" spellCheck={false} className="field mt-2" value={agentKey} onChange={(e) => setAgentKey(e.target.value)} placeholder="gokui_live_…" /></div><button type="button" className="btn-ink" onClick={probeGate} disabled={!agentKey}>{t("skill.request")}</button></div><p className="text-xs text-dim mt-3">{t("skill.probeNote")}</p>{probe && <pre className="code-block mt-5 max-h-[360px] overflow-auto" role="status">{JSON.stringify(probe, null, 2)}</pre>}</div>
    </main>
  );
}

export default function SkillPage() { const { t } = useI18n(); return <Suspense fallback={<main className="p-10 mono" role="status">{t("common.loading")}</main>}><SkillDetail /></Suspense>; }
